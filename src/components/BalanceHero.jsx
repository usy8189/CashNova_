            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                ...
                <div className="flex flex-row gap-4 items-center">
                    <div className="flex items-center gap-3 bg-emerald-500/8 border border-emerald-500/10 rounded-xl px-6 py-4">
                        ...
                        <div>
                            <p className="text-[11px] text-emerald-400/60 font-medium uppercase tracking-wider">Income</p>
                            <p className="text-base font-bold text-emerald-400">{formatCurrency(income)}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3 bg-red-500/8 border border-red-500/10 rounded-xl px-6 py-4">
                        <ArrowDownRight size={20} className="text-red-400" />
                        <div>
                            <p className="text-[11px] text-red-400/60 font-medium uppercase tracking-wider">Expenses</p>
                            <p className="text-base font-bold text-red-400">{formatCurrency(expense)}</p>
                        </div>
                    </div>
                    {todayExpense > 0 && (
                        <div className="flex items-center gap-3 bg-white/[0.03] border border-white/[0.06] rounded-xl px-6 py-4">
                            <TrendingDown size={20} className="text-white/40" />
                            <div>
                                <p className="text-[11px] text-white/30 font-medium uppercase tracking-wider">Today</p>
                                <p className="text-base font-bold text-white/70">{formatCurrency(todayExpense)}</p>
                            </div>
                        </div>
                    )}
                </div>
